...
void thread_unblock(struct thread *t)
{
  ...
  list_insert_ordered(&ready_list, &t->elem, comparador_pri, NULL);
...
void thread_yield(void)
{
  ...
  old_level = intr_disable();
  if (cur != idle_thread)
    list_insert_ordered(&ready_list, &cur->elem, comparador_pri, NULL);
...
void func_dormir_threads(int64_t ticks)
{
  ...
  list_insert_ordered(&lista_de_threads_en_espera, &thread_actual->elem, comparador_pri, NULL);
  thread_block();
